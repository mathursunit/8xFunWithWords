...
try{
  (function(){
    ...
    const state = ANSWERS.map(_ => ({ rows: Array(MAX_ROWS).fill(""), attempt:0, solved:false, invalidRow:-1 }));
    const boardsEl = document.getElementById("boards");
    const keyboardEl = document.getElementById("keyboard");
    const boardNumEl = document.getElementById("boardNum");
    const resetBtn = document.getElementById("resetBtn");

    // Build dictionary from bundled JS list first to avoid file:// fetch issues
    let validSet = new Set((window.VALID_WORDS || []).map(w=>w.toUpperCase()));
    ANSWERS.forEach(w=>validSet.add(w.toUpperCase()));

    // Optional: try to extend from assets/valid_words.txt when running on http(s)
    try{
      if (location.protocol.startsWith("http")){
        fetch('assets/valid_words.txt').then(r=>r.ok?r.text():"").then(t=>{
          if (!t) return;
    ...
    function onEnter(){
      ...
      if (guess === answer){
        ...
        if (activeBoard === 7){
          ...
          setTimeout(()=>alert("🎉 You solved all 8!"), 50);
        } else {
          unlockNext();
        }
      } else {
        ...
        if (s.attempt >= MAX_ROWS){
          setTimeout(()=>alert("Out of tries! Word was " + answer), 10);
          unlockNext();
        }
      }
    }
    ...
    function paintRow(bi, ri, res){
      ...
      for (let i=0;i<WORD_LEN;i++){
        ...
        (function(tt,cls){ setTimeout(()=>{ tt.classList.remove("flip"); tt.classList.add(cls); }, 80+i*30); })(t,res[i]);
      }
    }
    ...
    function updateStatus(){
      boardNumEl.textContent = (activeBoard+1);
    }
